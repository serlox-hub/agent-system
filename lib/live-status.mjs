...
// lib/lane-model.mjs's snapshot and notifications, and on into every view
// that renders them (`status` as a STATES/NOTIFY lookup key,
...
// that looks like plain internal state. C1 (\x80-\x9f) goes too: \x9b is an
// 8-bit CSI. Also the event log's read-side strip: `EventTail`
// (lib/event-fold.mjs) runs every event string through this, so a change here
// changes both trust boundaries.
export function stripControlBytes(s) {

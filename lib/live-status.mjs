...
export function readLiveStatuses() {
  ...
  for (const file of files) {
    ...
    // file itself carries no `sessionId`, so every entry this function
    // returns has one real value to key on (Phase 2's ordering tiebreak,
    // Phase 5's per-session notification dedup).
    const sessionId = typeof data.sessionId === 'string' ? stripControlBytes(data.sessionId) : String(data.pid);

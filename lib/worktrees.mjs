 ...
 * single formula `bin/lanes.mjs`'s `list` and `ui/dashboard.mjs`'s MARKS
 * column both render, each applying its own colour and width handling.
 *
...
export function planCreate(config, name) {
  ...
  if (!raw) return { error: 'worktreesDir is not configured — set it in .claude/agent-system.json' };
  ...
  // callers (list/doctor/ui) — correct there, but `new` is itself an act of
  ...
  if (!existsSync(raw)) {
    if (!existsSync(dirname(raw))) {
      return {
        error: `worktreesDir ${raw} does not exist, and neither does its parent — check .claude/agent-system.json`,
      };
    }
    try {
  ...
  return { path: join(raw, name), lane, renumbered, createdDir };
}

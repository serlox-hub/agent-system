...
export function enumerateLanes(config) {
  ...
  return laneNames(config).map((name) => {
    ...
    // literal "HEAD", with isBase: true, so list/ui/MARKS need no
    // per-renderer special-casing for it. No extra subprocess needed: an
 ...
 */
export function removeWorktree(config, lanes, { force = false } = {}) {

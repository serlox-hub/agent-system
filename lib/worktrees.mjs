 ...
 * timeout each, `lib/git.mjs`), so it blocks its caller — `lib/lane-model.mjs`'s
 * `createLaneSource` throttles it to one refresh per 20 `advance()` calls
 * rather than running it on every redraw.
 */
export function enumerateLanes(config) {
  ...
  return laneNames(config).map((name) => {
    ...
    // `isBase` widens below to include a lane merely *detached* at base's commit
    // (what every `lanes new`/`reset` leaves behind, harmless to commit on top
    // of). `holdsBaseBranch` stays narrow: the worktree has the base branch
    // itself checked out — which git permits in at most one worktree at a
    // time — so a commit here moves the shared branch, not just this lane's.
    // `laneMarks` flags that case separately for exactly that reason.
    const holdsBaseBranch = rawBranch === base;
    let isBase = holdsBaseBranch;
...
/**
 * Divergence and identity tokens for a lane vs its base, as `{ text, tone }`
 * pairs — the single formula `lib/lane-model.mjs` puts in every snapshot lane
 * and `ui/dashboard.mjs`'s BRANCH cell renders, applying its own colour and
 * width handling.
 *
 * `on-base!` fires first, ahead of divergence: a lane holding the base
 * branch itself is a different kind of risk than being dirty or ahead — a
 * commit there moves the shared branch, not just this lane's. It also
 * pre-empts `free` below even when `isFree` is true for that lane — this
 * marker warns about committing here, `isFree` answers a different question
 * (would taking the lane over lose anything), and the two are allowed to
 * disagree.
 *
...
export function laneMarks(lane) {
  const tokens = [];
  if (lane.holdsBaseBranch) tokens.push({ text: 'on-base!', tone: 'danger' });
 ...
 * `origin/<base>` after a fresh fetch, deleting the branch just vacated if
 * `git branch -d` accepts it and reporting it as `branchKept` otherwise.
 *
 * Two things refuse even with `force`, because `git checkout --detach` would
 * not keep them safe: uncommitted changes (carried over, so the lane comes out
 * detached at base but still dirty — not the clean state this promises, and
 * still not free) and commits on a detached HEAD that no ref holds (orphaned
 * into the reflog). `force` only overrides commits not in `origin/<base>`,
 * which is what a squash or rebase merge leaves behind — the routine case, so
 * the flag gets typed by habit, and making it discard work too would turn that
 * habit into data loss.
 */
export function resetLane(config, lane, { force = false } = {}) {
  ...
  if (lane.dirty) {
    return {
      error: `lane ${lane.lane} (${lane.name}) has ${lane.dirtyCount} uncommitted change(s) — commit, stash or discard them first; --force only overrides commits not in origin/${base}.`,
    };
  }
  if (!force && !isFree(lane)) {
    return {
      error: `lane ${lane.lane} (${lane.name}) is not free — ${lane.ahead} commit(s) not in origin/${base}. Use --force to reset anyway (e.g. after a squash or rebase merge).`,
    };
  }
  ...
  // Counted against every ref after the fetch, not `ahead`: `new --from
  // origin/feat/x` sits on commits a remote ref already holds. An empty answer
  // means git failed, and refuses too rather than guess.
  if (lane.branch === 'HEAD') {
    const orphans = gitLine(lane.path, ['rev-list', '--count', 'HEAD', '--not', '--branches', '--remotes', '--tags']);
    if (orphans !== '0') {
      return {
        error: orphans
          ? `lane ${lane.lane} (${lane.name}) has ${orphans} commit(s) on a detached HEAD that no branch holds — \`git branch <name>\` in the lane first; reset would orphan them, --force or not.`
          : `could not check lane ${lane.lane} (${lane.name}) for commits no branch holds — refusing rather than risk orphaning them.`,
      };
    }
  }
  ...
  if (outgoingBranch) {
    // `-d`, not `-D`: git deletes only a branch fully merged into its
    // upstream, or into HEAD — now `origin/<base>` — when it has none.
    // `/architect` pushes with `-u`, so its branches are usually judged
    // against their own remote copy and deleted, squash-merged or not —
    // nothing lost. Anything refused is reported as kept, never
    // force-deleted: after a squash or rebase merge, that happens only when
    // the branch has no upstream or its `origin/<branch>` copy is gone.
    const del = gitTry(lane.path, ['branch', '-d', outgoingBranch]);
    branchDeleted = del.ok ? outgoingBranch : null;
  }
  const branchKept = outgoingBranch && !branchDeleted ? outgoingBranch : null;

  return { lane: lane.lane, name: lane.name, branch: base, branchDeleted, branchKept };
}

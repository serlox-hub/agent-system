/**
 * The lanes data model: what `lanes status` shows, decided as data.
 * `buildSnapshot` joins the folded log (lib/event-fold.mjs) with git, the
 * live-status files and transcripts into one plain-JSON snapshot;
 * `createLaneSource` is the watch loop's refresh cycle around it, and decides
 * which events and transitions earn a desktop notification. No terminal
 * knowledge: ui/dashboard.mjs renders a snapshot, ui/status.mjs runs the loop.
 *
 * A source lives as long as that loop — meant to be weeks:
 *   - Bounded memory. Every piece of state a source keeps needs an eviction
 *     path: `prevLiveEv` is swept every tick (`liveTransitionNotifications`),
 *     and each 20-tick refresh rebuilds `ctxInfo` rather than growing it and
 *     prunes `state.sessionHistory` to the sessions on screen.
 *   - Never crash, never lose an event. `advance()` can still throw —
 *     `paint()` (ui/status.mjs) catches it, costing that frame — so whatever a
 *     tick has consumed goes out before anything that can throw: raw-event
 *     notifications before the fold (#19), and a throwing notifier costs
 *     neither the fold nor the notifications after it.
 */
 ...
 * (ui/dashboard.mjs) gives: `__proto__` resolves to a non-callable and
 * throws, and inherited methods such as `constructor` and `toString` are
 * callable and return a truthy value that is sent as a notification reading
 * `[object Object]`.
 */
const NOTIFY = {
 ...
 * key in lib/event-fold.mjs, renderSnapshot's `·` placeholder in
 * ui/dashboard.mjs) — worktree is the one identifier that always exists, since
 * resolveLane (lib/context.mjs) always sets it from the directory basename
 * even when lane is null. The notification title follows the same fallback so
 * it is never unidentifiable.
 */
...
function rowsFor(ctx, lanes, laneInfo) {
  ...
  for (const l of laneInfo) {
    ...
    rows.push({
      ...
      // could not be read — matching `resolveContext` (lib/context.mjs), which
      // derives the issue from the branch alone with no fallback. If the
      // branch *was* read and simply encodes no issue (e.g. back on base),
      // that is the truth: keeping a stale issue would contradict a fresh
      // "free" MARKS on the same row, since neither this issue nor this
      // branch matches it.
  ...
  // outright (see applyEvents, lib/event-fold.mjs), so what is left here is
  // either genuinely outside `worktreesDir` or was removed some other way
  // (manual `rm -rf`, `git worktree remove`) with no event to say so.
  // `existsSync` on the path recorded with the event is the backstop for that
  // second case — exact, not name-matched, and works for any project since it
  // needs no git call scoped to the current repo. Events older than the `path`
  // field have none and fail open, same as anything we simply can't verify.
 ...
 * per-row-per-tick read assumed. `printStatus` (ui/status.mjs) has no tick
 * loop to throttle against, so it omits `ctxInfo` and reads fresh — a
 * one-shot snapshot.
 *
 ...
 * render (`STATES.agent_end`, ui/dashboard.mjs) is byte-identical to `busy`,
 * so overriding it loses nothing, and protecting it left the
 * interrupted-mid-subagent case — no `Stop` ever fires, so the row would
 * otherwise stay stuck exactly like the bug #12 exists to fix.
 */
 ...
 * by subtraction: a future state added there can as easily be session-scoped
 * as lane-wide (`agent_start`, already there, is session-scoped), so deriving
 * "suppress" as the default direction would silently hide a live session the
 * moment someone adds a session-scoped protected state — the opposite of
 * what #14 exists to fix. Spelling it out instead makes "also suppress extra
 * rows" an opt-in edit made right here, next to the reasoning above.
 */
 ...
 * the running service — and nothing about how it looks. `renderSnapshot`
 * (ui/dashboard.mjs) is the terminal's view of it; another view reads the
 * same object instead of re-deriving these rules, which is how two copies of
 * them would drift.
 *
...
export function createLaneSource(ctx, { onNotify = () => {} } = {}) {
  ...
  // refreshed only every 20 `advance()` calls (~20s at the 1 Hz cadence of
  // `watchStatus`, ui/status.mjs) rather than on every one — including which
  // worktrees exist, so a lane created or removed elsewhere can take up to
  // ~20s to appear/disappear here. Setup-time action, not a per-task one, so
  // that lag is accepted rather than paid for on every tick.
  let tick = -1;
  let laneInfo;
  ...
  let liveStatuses;
  // `notify()` (ui/status.mjs) swallows its own failures; every onNotify gets
  // the same contract here, so a failing notifier costs neither the fold of
  // events already read nor the rest of the tick's notifications.
  ...
  return {
    advance() {
      ...
      if (tick % 20 === 0) {
        ...
        // see `pruneSessionHistory`'s own docstring (lib/event-fold.mjs) for why.

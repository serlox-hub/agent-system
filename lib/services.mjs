...
export function resolveServices(config, lane) {
  ...
  return declared.flatMap((svc, i) => {
    ...
    // Keyed by worktree NAME, never by lane number: lane numbers are positional
    // and shift when a lane is added or removed, which would leave a running
    // service's bookkeeping attached to a different worktree.
 ...
 * the computed port follows the lane number, and a lane can be renumbered while
 * the process is still running — the truth is what it actually bound to.
 */
 ...
 * the two disagree — a lane can be renumbered while the process stays up, so
 * the bound port and a fresh computation can diverge without the process

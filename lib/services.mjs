 ...
 *
 *   "dev": { "services": [
 *     { "name": "web", "command": "pnpm run dev --port {port}", "portBase": 300 },
 ...
 *
 * Kept out of its one caller, `lib/lane-model.mjs`'s `serviceFor` (D24), as
 * its own pure function so each branch — stopped, running-and-matching,
 * running-and-diverged, pidfile-with-no-recorded-port — gets direct test
 * coverage instead of needing a real pidfile and a live pid per branch.
 */

 ...
 *
 * Pulled out of the SERVICE cell in `ui/dashboard.mjs` (`lanes status`) as its
 * own pure function so each branch — stopped, running-and-matching,
 * running-and-diverged, pidfile-with-no-recorded-port — gets direct test
 * coverage instead of only exercising it indirectly through the renderer.
 */

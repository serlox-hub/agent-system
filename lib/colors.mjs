...
/** `lane -> '#RRGGBB'`: the developer's override, else the default palette; `null` for no lane. */
export function laneHexFor(overrides = readColors()) {
  return (lane) => {
    if (!lane) return null;
    if (overrides[lane]) return `#${overrides[lane]}`;
    return DEFAULT_PALETTE[(lane - 1) % DEFAULT_PALETTE.length] ?? null;
  };
}

 ...
 * are single-line JSON well under that: `detail` is capped at 300 *characters*
 * to keep the log small, but the byte check in `emit` below — not that cap —
 * is what actually enforces the limit, since 300 characters can be well over
 * 300 bytes once the content is multi-byte UTF-8.
 */
...
export function emit(event) {
  try {
    ...
    rotateIfNeeded();
    // Hook-driven emitters (hooks/emit.mjs, hooks/commit-guard.mjs) always pass
    // an explicit `session` key sourced from their own hook payload, even when
    // its value is null — so they always win here and never fall through to
    // env. CLI-driven emitters (bin/lanes.mjs, via emitWithContext) never pass
    // the key at all, since a plain `lanes` invocation has no hook payload to
    // read one from; those fall back to CLAUDE_CODE_SESSION_ID, which Claude
    // Code sets on every subprocess it spawns, or null outside Claude Code.
    const hasOwnSession = Object.prototype.hasOwnProperty.call(event, 'session');
    const record = {
      ts: Date.now(),
      ...event,
      session: hasOwnSession ? event.session : process.env.CLAUDE_CODE_SESSION_ID || null,
    };
    if (typeof record.detail === 'string' && record.detail.length > 300) {
      record.detail = `${record.detail.slice(0, 297)}...`;
    }
    const line = `${JSON.stringify(record)}\n`;
    // PIPE_BUF is a byte limit — `.length` counts UTF-16 code units, which
    // undercounts multi-byte UTF-8 content (CJK, accents, emoji) and would let
    // a line past 4096 real bytes through while still reading "under 4000".
    if (Buffer.byteLength(line, 'utf8') > 4000) return false; // refuse rather than risk an interleaved write

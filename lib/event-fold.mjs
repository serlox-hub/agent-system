...
// reader goes through `EventTail` — not in `emit()`: the dashboard replays logs
// written by older builds, which a write-side strip never reaches. Paths stay
// raw, like live-status's `cwd`: they are matched against the filesystem,
// never printed.
...
export class EventTail {
  ...
  read() {
    ...
    for (const line of lines) {
      ...
      try {
        out.push(JSON.parse(line, stripEventStrings));
